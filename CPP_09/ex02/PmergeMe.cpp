...
{
	// Flatten(convert) tmp.elements into a single list
...
void PmergeMe::sort(int argc, char **argv)
{
	...
	{
		if (!isNum(argv[i]))
			throw std::runtime_error("Error: Please use positive integers only");
		else if (atof(argv[i]) > INT_MAX)
			throw std::runtime_error("Error: Number bigger than INT_MAX found");
		std::list<int> list;
